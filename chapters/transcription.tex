...
\section{Convolutional Recurrent Neural Networks}\label{sec:crnn}
	...
	\subsection{Training and evaluation}
		...
		\subsubsection*{Evaluation}
			...
			\end{aligned}
			\]
			where the \(\operatorname{dist}()\) function evaluates the \emph{un-normalised} edit distance and \(\delta\) outputs 1 if its arguments are the same, letter for letter, or 0 when they differ. Note that, unless otherwise specified, we use the raw values, without discounting for placeholders in the ground truth. This results in minimal attainable error rates that are above zero. \todo{Give values}.
...
\section{Experiments}\label{sec:transcription_experiments}
	...
	\subsection{Academic databases}
		At the very first phase, we validated that the architecture can learn and perform handwriting recognition by training on the already-discussed RIMES database. We trained and predicted using \emph{binarised} images of single words, hence the name \ds{Words} model \todo{Also specify non-binary model}. The model converged after seeing \(\approx 78\)k training examples to a validation \(\cer = 0.35\), but it did not generalise to our document, where \todo{\(\cer = what\)}. Since at first we did not use dropout, we next tried a model trained on the same dataset with dropout probability \(p_\mathit{drop} = 0.2\). It achieved similar performance on validation dataset, but it generalised better to real world data, achieving \(\cer = 6.5\). \todo{While this is far from being usable in practice, the small performance improvement demonstrates that transfer learning is a valid strategy, so we will continue to explore it.}
