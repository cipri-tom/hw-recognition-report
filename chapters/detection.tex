...
There are two important observations about the above measure. First, a great importance is given to the confidence score, since recall and precision are defined by only considering examples ranked above a given rank. Second, it does \emph{not} define what a positive result means. For object detection, a predicted bounding box \(B_p\) is considered positivite if it overlaps a ground-truth box \(B_{gt}\) with \(\iou(B_p, B_{gt}) > 0.5\), where \[
...
\section{Generating data for training experiments}\label{sec:detection_experiments}
	...
	To perform intermediate evaluations of trained models we used the standard PASCAL VOC measure which scores them according to the average precision (AP) at 0.5 overlap (\(\operatorname{AP@}0.5\operatorname{IoU}\)). This means that each predicted box is considered a true positive if
	...
	\subsection{Collage of paragraphs}
		...
		A model trained on this dataset converged rather quickly, in \(\approx 8000\) iterations. On its validation data, the model achieves a remarkably high performance, with an average precision \(\)
