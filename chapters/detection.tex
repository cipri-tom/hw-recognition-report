...
\section{Faster R-CNN}\label{sec:faster_rcnn}
	...
	\subsection{Training}\label{sec:frcnn_train}
		...
		We feed the network one image at a time, resized such that its shorter side is \(600\mathit{px}\) wide. Note, however, that a minibatch is formed of bounding boxes, so many candidates can be generated from a single input image and a set of anchors. Several hyper-parameters control the composition of the batches:
		\begin{description}
...
\section{Connectionist Text Proposal Network}\label{sec:ctpn}
	...
	\subsection{RPN becomes CTPN}
		...
		\begin{figure}
			\begin{subfigure}[c]{.69\linewidth}
				...
				\caption{Overview}\label{fig:ctpn_arch}
			\end{subfigure}
			\begin{subfigure}[c]{.30\linewidth}
				...
				\end{subfigure}
			\end{subfigure}
			\caption[The Connectionist Text Proposal Network]{The Connectionist Text Proposal Network:
	...
	\subsection{Training}
		Once the \CTPN{} predicts fine-scale proposals, these are classified into \texttt{text} or \texttt{background} by the same Fast R-CNN core as before. However, in this case we use a different feature extractor, namely the VGG network \citep{VGG}, in order to reproduce the setup of the original work. As in the previous case, we do not train from scratch but instead we start with the weights provided by the authors, who used 3000 images of text in the wild.
...
\section{Datasets and experiments}\label{sec:detection_experiments}
	...
	\subsection{RIMES-filled templates}
		...
		\subsubsection*{\ds{Template} and \ds{Template_bin} datasets}
...
\section{Faster R-CNN Results}\label{sec:frcnn_results}
	\todo{ In addition, having a high recall is more important than predicting with a high confidence, since the transcription phase can easily ignore false positives. For this reason, the comparison in \autoref{sec:frcnn_results} will consider these two criteria for comparing different experiments. }
...
%========================================================================================
	...
	\subsection{Results}
		\begin{figure}[ht]
			\begin{subfigure}{.49\linewidth}
				\includegraphics{ctpn_vs_frcnn_recall}
				\caption{}
				\label{fig:ctpn_vs_frcnn_recall}
			\end{subfigure}
			\begin{subfigure}{.49\linewidth}
				\includegraphics{ctpn_vs_frcnn_prec}
				\caption{}
