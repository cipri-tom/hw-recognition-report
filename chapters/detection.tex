...
\section{\FRCNN{} Results}\label{sec:frcnn_results}
	...
	In conclusion, we observe that \FRCNN{} is able to learn handwriting specific features and can use them to detect handwritten text reasonably well. For our model the confidence score of the detected boxes is less helpful in filtering them, but this is a small problem since false negatives can be filtered at a later time. Alternatively, it is likely that training for more steps would aid the model to better distinguish between text and text-like examples.
